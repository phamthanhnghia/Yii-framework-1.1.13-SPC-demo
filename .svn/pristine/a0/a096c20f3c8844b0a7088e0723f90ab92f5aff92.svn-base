...
<h1>Phân Quyền Cho Role: <?php echo $mGroup->role_name ?></h1>
        ...
        <div class="wrap_privilege">
            ...
            <ul class="permission-list">
                <?php foreach($aController['actions'] as $keyAction=>$aAction): ?>
                    <?php $classCheck = ''; ?>
                <li><input type="checkbox" name="<?php echo $keyController.'['.$keyAction.']' ?>" value="1" id="<?php echo $keyController.'_'.$keyAction ?>" <?php if(in_array($keyAction, $aActionsAllow)) {echo 'checked="checked"'; $classCheck = 'hight_light_background';} ?>>
