<?php $mFileScan = GasSalesFileScan::model()->findByPk($model->file_scan_id); ?>
<div id="printElement">
...
<ul class="clr">
    <?php foreach($mFileScan->rFileScanDetail as $key=>$item):?>
        <?php $link = Yii::app()->createAbsoluteUrl('admin/ajax/viewImageProfileHs', array('id'=>$item->id, 'model'=>'GasSalesFileScanDetai'));
            $class='';
            if($item->id==$model->id)
                $class='hight_light';
        ?>
